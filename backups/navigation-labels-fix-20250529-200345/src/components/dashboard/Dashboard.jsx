...
const Dashboard = ({ user }) => {
  ...
  const handleRunReport = (reportConfig) => {
    console.log('Dashboard handleRunReport called with config:', reportConfig)
    console.log('Total time entries available:', allTimeEntries.length)
    ...
    try {
      ...
      if (reportConfig.startDate) {
        filtered = filtered.filter(e => new Date(e.date) >= new Date(reportConfig.startDate))
        console.log('After start date filter:', filtered.length)
      }
      if (reportConfig.endDate) {
        filtered = filtered.filter(e => new Date(e.date) <= new Date(reportConfig.endDate))
        console.log('After end date filter:', filtered.length)
      }
      if (reportConfig.jobs.length > 0) {
        filtered = filtered.filter(e => reportConfig.jobs.includes(e.job_address))
        console.log('After jobs filter:', filtered.length, 'jobs:', reportConfig.jobs)
      }
      if (reportConfig.tasks.length > 0) {
        filtered = filtered.filter(e => reportConfig.tasks.includes(e.csi_division))
        console.log('After tasks filter:', filtered.length, 'tasks:', reportConfig.tasks)
      }
      if (reportConfig.workers.length > 0) {
        filtered = filtered.filter(e => reportConfig.workers.includes(e.user_id))
        console.log('After workers filter:', filtered.length, 'workers:', reportConfig.workers)
      }
      ...
      if (reportConfig.reportStyle === 'cards') {
        console.log('Generating card-style report with', filtered.length, 'entries')
        ...
        console.log('Generating table-style report with', results.length, 'rows')
  ...
  const openTableReportTab = (data, headers) => {
    console.log('Opening table report with', data.length, 'rows')
    const reportData = {
      results: data,
      headers: headers,
      style: 'table'
    ...
    console.log('Stored report data in sessionStorage with ID:', reportId)
    
    const url = `/report?id=${reportId}`
    console.log('Opening new window with URL:', url)
    const newWindow = window.open(url, '_blank')
    if (!newWindow) {
      alert('Please allow pop-ups for this website to view reports in a new tab.')
    }
  }

  const openCardReportTab = (filteredData, groupingFlags) => {
    console.log('Opening card report with', filteredData.length, 'entries')
